...
export default async function SubjectProfile({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = await params;
  const subject = subjectsData.find((item) => item.id === resolvedParams.id);
  const career = careersData.find((item) => item.id === "sistemas");
      ...
      <div className="relative z-10 max-w-4xl mx-auto w-full px-4 sm:px-6 py-6">
        <div className="mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center text-sm text-[#A89F95] gap-1.5 flex-wrap">
            ...
            <Link href="/carreras/sistemas" className="hover:text-[#4A7A52] transition-colors">
              {career?.shortName}
            </Link>
            <ChevronRight className="w-3.5 h-3.5" />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#A89F95]">Material compartido por la comunidad</p>
                </div>
              </div>

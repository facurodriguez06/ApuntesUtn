...
function generateId(prefix, name) {
  let cleanName = name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  return `${prefix.substring(0,3)}-ele-${cleanName}`;
}
...
for (const [career, subjects] of Object.entries(electivas)) {
  ...
  // Find where the curriculum array ends for this specific career
  const searchRegex = new RegExp(`(${career}:\\s*\\{[\\s\\S]*?curriculum:\\s*\\[[\\s\\S]*?)(\\],\\s*color:|\\n\\s*\\],\\s*name:)`);
  ...
  if (match) {
    ...
    for (const s of subjects) {
      const id = generateId(career, s.name);
      newContent += `      {
        id: '${id}',
        title: '${s.name}',
        year: ${s.year},
        regulares: [], // Requerimientos regulares vacíos por defecto
        aprobadas: [], // Requerimientos aprobados vacíos por defecto
        isElectiva: true,
        icon: <GraduationCap className="w-5 h-5 opacity-80" />
      },\n`;
    }
    
    content = content.replace(searchRegex, `$1${newContent}$2`);
  }
}

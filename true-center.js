...
async function processImage() {
  ...
  // Usamos el mismo tamaño grande del icono que antes para mantener la legibilidad
  const cropSize = Math.floor(Math.min(width, height) * 0.75); 
  const halfSize = Math.floor(cropSize / 2);
  
  // El usuario noto que hay menor espacio azul a la derecha que a la izquierda.
  // Esto significa que el libro se ve muy pegado a la derecha.
  // Vamos a desplazar el marco de recorte hacia la derecha un 6%.
  // Al mover el "lente" hacia la derecha, la imagen grabada queda desplazada hacia la izquierda.
  ...
  // Evitar salir del marco de la imagen original
  ...
  console.log("Corrección geométrica aplicada. Libro centrado manualmente.");
}
